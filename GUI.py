...
def QueryModifier(Query):
    ...
    questions_words = ["how", "what", "who", "where", "when", "why", "which", "whose", "whom", "can you", "what's",
                       "where's", "how's"]

    if any(word + " " in new_query for word in query_words):
        if questions_words[-1][-1] in ['.', '?', '!']:
...
def SetMicrophoneStataus(Command):
...
def MicButtonInitialed():
    SetMicrophoneStataus("True")

def MicButtonClosed():
    SetMicrophoneStataus("False")


def GraphicsDirectoryPath(Filename):
    Path = rf'{GraphicsDirpath}\{Filename}'
    if not os.path.exists(Path):
        print(f"Warning: Graphics file {Path} not found")
    return Path
...
class ChatSection(QWidget):
    ...
    def update_mic_status(self):
        status = self.SpeechRecogText()
        self.update_mic_icon(status)

    def update_mic_icon(self, is_active):
        icon_path = GraphicsDirectoryPath("Mic_on.png" if is_active else "Mic_off.png")
...
class CustomTopBar(QWidget):
    ...
    def initUI(self):
        ...
        self.resotre_icon = QIcon(GraphicsDirectoryPath('Minimize.png'))
    ...
    def maximizedWindow(self):
        if self.parent().isMaximized():
            ...
            self.maximize_button.setIcon(self.resotre_icon)
